import { Element, MapElementInterface } from "../../types"
...
export function SpaceEditor({ name, thumbnail, dimension }: MapEditorProps) {
  ...
  const [elements, setElements] = useState<MapElementInterface[]>([]);
  ...
  async function createSpace() {
    ...
    const res = await fetch(`${BACKEND_URL}/space`, {
      ...
      body: JSON.stringify({ name, dimension, thumbnail }),
    });
  ...
  const handleElementDrop = useCallback((element: Element) => {
    if (isEditMode) {
      const [width, height] = dimension.split('x').map(Number);
      const { id, type, position: _, ...rest } = element;
      const newElement: MapElementInterface = {
        id,
        type,
        position: {
          x: width / 2,
  ...
  return (
    <div className="flex flex-col gap-4 mt-8 border-t pt-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div id="phaser-game" className="p-3 col-span-3 bg-gray-100 rounded-lg overflow-scroll" />

        {isEditMode && (
          <div className="w-64 bg-gray-100 p-4 overflow-y-auto col-span-1 rounded-lg">
            ...
            <div className="space-y-4">
              {availableElements.map((element) => (
                ...
                >
                  <div className={`absolute top-2 right-2 w-3 h-3 rounded-full ${element.static ? 'bg-red-500' : 'bg-green-500'}`} />
                  <img src={element.imageUrl} alt={element.name} className="w-full h-32 object-contain mb-2" />
