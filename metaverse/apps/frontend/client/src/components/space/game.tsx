...
import { playerPositionAtom } from "../../lib/atom/playersPositionAtom"
...
import { OtherUser, SpaceElementsState } from "../../types/index"
...
export function Game({ otherPlayersRef }: { otherPlayersRef: React.RefObject<OtherUser[]> }) {
    ...
    const elementsState = useRecoilValue<SpaceElementsState>(spaceElementsAtom);
    const elements = Object.values(elementsState);
    const playerPosition = useRecoilValue(playerPositionAtom);
    const userName = useRecoilValue(userNameAtom) || "Unknown";

    const dimensionsStr = typeof dimensions === 'string' ? dimensions : `${dimensions.width}x${dimensions.height}`;
    ...
    useEffect(() => {
        ...
        function create(this: Phaser.Scene) {
            ...
            elements.forEach((item) => {
                ...
                element.setData("static", item.static || false);

                if (item.static) {
                    element.setImmovable(true);
                }
                
                const body = element.body as Phaser.Physics.Arcade.Body;
                if (body) {
